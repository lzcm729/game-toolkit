...
- **`page.status` 横幅。** 原版落地后在裁决单页顶加一行状态（提交号、账本条目号），页面是活的，
  用户回头一看就知道这张单落没落。4.8.0 把页面当成构建产物，这行没了。现在 `裁决单.json`
  的 `page.status` 出单时留空，落地后填上重建，页顶渲染成横幅；`build_page.py` 校验它是字符串。
- **来源页按结论更新。** 原版收尾是「条目来源的那张页（盘点页、状态机页）按结论更新」，
  4.8.0 缩成了「条目号回写到来源报告对应条目旁」。改回原话，评审报告、修正清单、盘点页、状态机页都算。
...
4.6.0 在「它查这些」里加了「完全陌生的字段只提示」，紧挨着原有的「丢掉会改变结果含义的
字段算错」。两句说的是两回事，但措辞没分开：

- 前者说的是**配置里的字段名本身**对不对（拼错、放错层）
- 后者说的是**字段名写对了，但这个后端不支持它**

各自补了一句点明区别。

### dry-run 的 `[plan]` 行只打模型，不打比例

要核对比例得自己去翻临时的 batch JSON —— 冷启动评测里 codex 就是这么干的。现在
`[plan]` 行带上生效的 `aspect_ratio`，条目级覆盖也如实显示。

### 两条弱测试

外部评审早就报过，一直没修：

- `test_model_with_image_gen_backend` 在「错误 + 治理 + 提示」拼起来的字符串里找子串，
  **任何严重度都能过** —— 4.0.0 起这里本该是错误，真降级成提示它照样绿。改成断言
  `report.errors` 里正好一条
- `test_unregistered_engine_prefix_...` 有一行重复断言，而且和 `test_engine_adapter.py`
  里那条是同一个测试。它独有的那句断言（「给出真正的出路」）搬进了后者，两条从端到端
  文件里删掉 —— 它们是 `engine_adapter` 的单元测试，本来就不该住在 `test_main_flow` 里

### 验证

581 passed（4.6.0 是 582：删了 2 条重复的、加了 1 条）。三条变异全部报红，其中收紧后的
那条弱测试单独确认过：把后端能力检查降回告警，它现在会红 —— 改之前不会。

**发版前按规矩跑了冷启动评测**（改了 SKILL.md）：两轮退码 0，17 项全过。真实运行里确认
`[plan]` 行现在打的是 `(model=gemini-3-pro-image aspect_ratio=1:1)`。
