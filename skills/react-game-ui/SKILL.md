...
name: react-game-ui
description: |
  Web 技术栈的游戏 UI 实现模式：shadcn/ui + Tailwind + Framer Motion。
  资源条、动画计数器、进度/仪表、卡牌、金币弹出、错误抖动、HUD 布局、
  响应式棋盘、模态与提示、动效可关闭、读屏播报、渲染性能。

  仅当项目确实用 React 做游戏界面时才用。它是**可选的技术适配**，不是核心：
  - 引擎无关的 UI 原则（可读性、焦点、输入适配、反馈、无障碍）→ game-ui-design
  - Godot / Unity / UE 的界面实现 → 用各引擎自己的 UI 系统，本 skill 不适用
  - 通用网页/应用界面（非游戏）→ frontend-design
---
...
> **这是实现层。** 该不该有这个元素、信息优先级怎么排、焦点与输入怎么适配、
> 反馈要多快 —— 那些是设计问题，去 `game-ui-design`。本文只回答「用 React
> 怎么把它做出来」。两者一起用：先定原则，再落实现。
