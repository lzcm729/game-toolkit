...
同一机制跨报告先对齐：核属主规格的承诺范围、现行裁决的日期与适用范围，再核代码事实。
若出现「补实现／删除／属主不承诺」等相互冲突的结论，记录各自 `系统#行号`、属主规格与
裁决 `file:line`，冲突未解决不得进入第四节「设计已定代码未跟」，放第二节等待属主判断。
未配置属主或未找到裁决时写未知；不能用某一份报告的 Missing 替另一个属主承诺需求。
文档的「已接通」被代码驳倒，只证明状态声明需纠正；派实现任务仍需独立的现行需求依据。
同样，不能从「未实现」反推「已裁定删除」。删除建议必须有明确废止裁决及其覆盖范围。
第四节每项都须给属主规格与现行裁决的核对结果；两者一致且明确承诺才允许派实现项。

Code-only 迁移只给「描述差异、待核机制变化」。结合两轮 code_ref 核同一条调用链与属主裁决，
不能把本轮/基线独有描述直接转成机制新增/消失，更不能据此安排补实现或删代码。
