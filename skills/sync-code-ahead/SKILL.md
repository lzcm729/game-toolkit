...
每条先经阶段 2 按固定快照核对，再经阶段 3b 候选复核后进入 pending；按 code_ref＋summary 与现有条目去重。
报告只提供线索，不代替源码与设计证据；出处不足时先补核，不编目标文档。

### 阶段 3b：候选复核与裁决路由

本轮新候选与全部旧 pending 都逐项复核：从固定 H 读源码行为与正式调用链，打开属主设计和
目标文档，读取 game-toolkit.yaml 的 `doc_feedback`；有 `rulings_ledger` 时按最新日期及适用范围
核裁决，有 `owners` 时据它定位属主。缺配置或冲突时明示未知，不把已有代码当成设计批准。
对每项记录 `code_ref / 属主规格 file:line / 裁决日期与 file:line（或无）/ 目标文档`，再分流：

| 复核结论 | 后续处理 |
|---|---|
| 获准补文档：属主明确承诺，现行裁决一致，目标确实漏记 | 可列入阶段 5 的同步候选，按用户确认执行 |
| 待设计判断：属主未承诺、证据不足或裁决冲突 | 保留 pending，`status: deferred`，在 defer_reason 记缺口；不得进入阶段 6 |
| 应回代码修正：行为违反现行属主规格或明确废止裁决 | 保留 pending 并记代码修正路由与依据，交正向流程；不靠改需求来消除差距 |

代码存在不等于获准写入需求，代码未实现不等于已裁定删除。「基于代码同步」不是越过设计裁决的许可。
用户选择「全部同步」只覆盖获准补文档项；待设计/代码修正项需先解决裁决，不借批量确认转正。
复核结果写入现有 evidence / defer_reason，不增加检查点 schema、不重分配 pending ID；
未经文档覆盖或源码撤销核验的条目继续保留，scan 与 pending 的恢复顺序不变。
...
- 如果代码实现与设计意图有差异，按阶段 3b 核属主与裁决，路由到待设计判断或代码修正；不得自动改需求
