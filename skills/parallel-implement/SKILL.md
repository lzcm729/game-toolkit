...
每个系统一个 worktree。创建后：
- **依赖准备** —— 按项目实际情况做，**不要无条件执行下面这条**。
  Node 项目 + Windows 可用 junction 共享 `node_modules`：
  ```bash
  cd {主项目上级目录} && cmd //c "mklink /J wt-{系统名}\node_modules {主项目目录名}\node_modules"
  ```
  `cmd` 只在 Windows 上存在；macOS / Linux 用 `ln -s`，或各自安装依赖。
...
用 Skill 工具调用 `game-toolkit:layer-contracts`，按其中的职责边界与 A 层交付门槛工作。
你的角色对应的是 {Framework / Content / Interface} 职责。

**本任务的契约：**
{从设计文档/采纳索引摘出的状态与所有权、命令与前置条件、转换与原子性、拒绝与恢复、
 事件与副作用、不变量；缺项在这里标明「契约缺失」}

契约有缺口时：能做的部分继续做，**不要自行补造规则**，把缺口写进完成报告交回主流程。
...
边界按**语义所有权**划，不是按目录划 —— `layer-contracts` 明确允许多个职责的实现
共处一个文件（Godot 里一个脚本同时承载规则与呈现很常见）。

**你负责：** {角色的语义职责，如「权威状态与命令处理」}
**你不碰：** {其他职责的语义范围，如「输入映射与反馈呈现」}
**参考位置：** {角色通常涉及的目录/文件，从 CLAUDE.md 读取；仅作定位提示，不是硬边界}

共处一个文件时：按语义所有权约定编辑区段并在报告里交接，
不要因为「文件不归我」而拒绝属于本职责的工作，也不要顺手改别人的语义。

## 完成后

1. 运行构建检查确保编译通过
2. **逐条核对本任务契约里的验收场景**，报告每条的实际观察结果。
   编译通过只说明能构建，不说明行为正确 —— 失败路径、并发、副作用尤其要给证据。
   无法验证的场景写明原因，不要跳过不提。
3. 契约缺口、以及你在实现中被迫做的任何行为决定，都要列出来交回主流程
4. 用 TaskUpdate 标记 Task #{N} 为 completed
5. 如果有后续依赖角色：用 SendMessage 发送给 {后续角色 agent name}，内容包括：
   - 你新增/修改了哪些类型和接口
   - 后续角色需要适配的部分
   - 注意事项
6. 保持在线，其他角色可能会有问题问你
